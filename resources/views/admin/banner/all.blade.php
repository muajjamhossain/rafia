            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-md-4 text-right">
                        <a href="{{url('dashboard/banner/add')}}" class="btn btn-md btn-primary waves-effect card_top_button"><i class="fa fa-plus-circle"></i> Add Banner</a>
                    </div>
                            ...
                            <table id="alltableinfo" class="table table-bordered custom_table mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($data->ban_image!='')
                                                <img class="table_image_ban" src="{{asset('uploads/banner/'.$data->ban_image)}}" alt="banner"/>
                                            @else
                                                <img class="table_image_ban" src="{{asset('uploads')}}/no-image-big.jpg" alt="banner"/>
                                            @endif
                                        </td>
                                        <td>
                                            @if($data->ban_publish==1)
                                                <a href="#" title="publish" id="unpublish" data-toggle="modal" data-target="#unPubModal" data-id="{{$data->ban_id}}"><i class="fa fa-check-square text-success fa-lg"></i></a>
                                            @else
                                                <a href="#" title="unpublish" id="publish" data-toggle="modal" data-target="#publishModal" data-id="{{$data->ban_id}}"><i class="fa fa-pause text-danger fa-lg"></i></a>
                                            @endif
                                            <a href="{{url('dashboard/banner/view/'.$data->ban_slug)}}" title="view"><i class="fa fa-plus-square fa-lg view_icon"></i></a>
                                            <a href="{{url('dashboard/banner/edit/'.$data->ban_slug)}}" title="edit"><i class="fa fa-pencil-square fa-lg edit_icon"></i></a>
...
<div id="publishModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/banner/publish')}}">
              @csrf
              <div class="card mb-0">
                <div class="card-header">
                    <h3 class="card-title float-left"><i class="fa fa-gg-circle"></i> Confirm Message</h3>
...
<div id="unPubModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/banner/unpublish')}}">
              @csrf
              <div class="card mb-0">
                <div class="card-header">
                    <h3 class="card-title float-left"><i class="fa fa-gg-circle"></i> Confirm Message</h3>
