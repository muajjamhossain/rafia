                    ...
                    <div class="col-12">
                        <div class="table-responsive">
                            <table id="alltableinfo" class="table table-bordered custom_table mb-0">
                                ...
                                <tbody>
                                    @php
                                        $all=App\Contactus::where('conus_status',0)->orderBy('conus_id','DESC')->get();
                                    @endphp
                                    @foreach($all as $data)
                                    <tr>
                                        ...
                                        <td>{{Str::words($data->conus_sub,7)}}</td>
                                        <td>{{$data->created_at->format('d-m-Y | h:i:s a')}}</td>
                                        <td>
                                          <a href="#" title="restore" id="restore" data-toggle="modal" data-target="#restoreModal" data-id="{{$data->conus_id}}"><i class="fa fa-refresh fa-lg restore_icon"></i></a>
...
<div id="restoreModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/contactus/restore')}}"/>
              @csrf
              <div class="card mb-0">
                <div class="card-header">
...
<div id="softDelModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/contactus/delete')}}"/>
              @csrf
              <div class="card mb-0">
                <div class="card-header">
                    <h3 class="card-title float-left"><i class="fa fa-gg-circle"></i> Confirm Message</h3>
