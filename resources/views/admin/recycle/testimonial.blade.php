    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="card-title card_top_title"><i class="fa fa-gg-circle"></i> Recycle All Testimonial</h3>
                    </div>
                    ...
                    <div class="col-md-7">
                        @if(Session::has('restore'))
                          <div class="alert alert-success alertsuccess" role="alert">
                             <strong>Successfully!</strong> restore testimonial information.
                          </div>
                        @endif
                        @if(Session::has('delete'))
                          <div class="alert alert-success alertsuccess" role="alert">
                             <strong>Successfully!</strong> permanently delete testimonial information.
                          </div>
                        @endif
                    ...
                    <div class="col-12">
                        <div class="table-responsive">
                            <table id="alltableinfo" class="table table-bordered custom_table mb-0">
                                ...
                                <tbody>
                                    @php
                                        $all=App\Testimonial::where('tm_status',0)->orderBy('tm_id','DESC')->get();
                                    @endphp
                                    @foreach($all as $data)
                                    <tr>
                                        <td>{{$data->tm_name}}</td>
                                        ...
                                        <td>{{Str::words($data->tm_review,5)}}</td>
                                        <td>
                                            @if($data->tm_photo!='')
                                                <img class="table_image_50bl" src="{{asset('uploads/testimonial/'.$data->tm_photo)}}" alt="photo"/>
                                            @else
                                                <img class="table_image_50bl" src="{{asset('uploads')}}/no-image-big.jpg" alt="photo"/>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="#" title="restore" id="restore" data-toggle="modal" data-target="#restoreModal" data-id="{{$data->tm_id}}"><i class="fa fa-refresh fa-lg restore_icon"></i></a>
                                            <a href="#" title="delete" id="softDelete" data-toggle="modal" data-target="#softDelModal" data-id="{{$data->tm_id}}"><i class="fa fa-trash fa-lg delete_icon"></i></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
<div id="restoreModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/testimonial/restore')}}"/>
              @csrf
              <div class="card mb-0">
                <div class="card-header">
