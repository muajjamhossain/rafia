...
class UserController extends Controller{
    ...
    public function edit($user){
        $allRole=UserRole::where('role_status',1)->orderBy('role_id','ASC')->get();
        $data=User::where('status',1)->where('username',$user)->firstOrFail();
    ...
    public function update(Request $request){
        ...
        $user=$request['user'];
        $id=$request['id'];
        $update=User::where('status',1)->where('username',$user)->update([
            'name'=>$request['name'],
            'phone'=>$request['phone'],
            'role_id'=>$request['role'],
            'created_at'=>Carbon::now()->toDateTimeString(),
        ...
        if($update){
            Session::flash('success','value');
            return redirect('dashboard/user/view/'.$user);
        }else{
            Session::flash('error','value');
            return redirect('dashboard/user/edit/'.$user);
        }
    }
