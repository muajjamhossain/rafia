...
class ContactUsController extends Controller{
    ...
    public function index(){
        $all=Contactus::where('conus_status',1)->where('conus_sub','Contact')->orderBy('conus_id','DESC')->get();
        return view('admin.contactus.all',compact('all'));
    }

    public function view($slug){
        $data=Contactus::where('conus_status',1)->where('conus_sub','Contact')->where('conus_slug',$slug)->firstOrFail();
        return view('admin.contactus.view',compact('data'));
    }

    public function softdelete(){
        $id=$_POST['modal_id'];
        $soft=Contactus::where('conus_status',1)->where('conus_sub','Contact')->where('conus_id',$id)->update([
            'conus_status'=>0,
    ...
    public function allprint(){
        $all=Contactus::where('conus_status',1)->where('conus_sub','Contact')->orderBy('conus_id','DESC')->get();
        return view('admin.contactus.allprint',compact('all'));
    }

    public function print($slug){
        $data=Contactus::where('conus_status',1)->where('conus_sub','Contact')->where('conus_slug',$slug)->firstOrFail();
        return view('admin.contactus.print',compact('data'));
    }
    ...
    public function pdf(){
        $all=Contactus::where('conus_status',1)->where('conus_sub','Contact')->orderBy('conus_id','DESC')->get();
        $pdf=PDF::loadView('admin.contactus.pdf', compact('all'));
